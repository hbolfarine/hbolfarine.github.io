...
israel = read.csv("week_14_docs/israel.csv")
...
library(ggplot2)
ggplot(israel, aes(x=cohort.size, y=read, col=ifelse(cohort.size <= 40, 
                                                     "Class kept intact", 
...
rdd1 <- lm(read ~ selection + treatment, data = israel)
summary(rdd1)
```
...
```{r, echo=FALSE, fig.height = 5, warning=FALSE, message=FALSE}
ggplot(israel %>% mutate(pred = predict(rdd1)),
